...
class ServeClientBase(object):
    ...
    def send_client_done_message(self):
        try:
            self.websocket.send(
                json.dumps({
                    "uid": self.client_uid,
                    "done_with_this_crap": True,
                })
            )
        except Exception as e:
            logging.error(f"[ERROR]: Sending data to client: {e}")
...
class ServeClientFasterWhisper(ServeClientBase):
    ...
    def transcribe_audio(self, input_sample):
        ...
        print("[INFO] I'm sending", len(input_sample), "to the gulag")
    ...
    def speech_to_text(self):
        ...
        while True:
            ...
            try:
                input_sample = input_bytes.copy()
                if self.last_input_sample is not None and input_sample.shape == self.last_input_sample.shape and np.all(input_sample == self.last_input_sample):
                    self.send_client_done_message()
                    time.sleep(0.5)
                    continue

                self.last_input_sample = input_sample
                result = self.transcribe_audio(input_sample)

                if result is None or self.language is None:
                    self.timestamp_offset += duration
                    time.sleep(0.05)    # wait for voice activity, result is None when no voice activity
                    continue
