...
class Client:
    ...
    def process_segments(self, segments):
        ...
        if self.log_transcription:
            ...
            print("[INFO] Transcript:", self.transcript)
            print()
            #text = text[-3:]
            #utils.clear_screen()
    ...
    def on_message(self, ws, message):
        ...
        if message.get("done_with_this_crap"):
            self.done_with_this_crap += 1
            return
    ...
    def wait_before_disconnect(self):
        ...
        while time.time() - self.last_response_received < self.disconnect_if_no_response_for:
            print("[INFO] Times", time.time() - self.last_response_received, self.disconnect_if_no_response_for)
            time.sleep(1)
            #continue
...
class TranscriptionTeeClient:
    ...
    def play_file(self, filename):
        ...
        with wave.open(filename, "rb") as wavfile:
            print("[INFO] wav sample width", wavfile.getsampwidth())
            print("[INFO] framerate", wavfile.getframerate())
            self.stream = self.p.open(
